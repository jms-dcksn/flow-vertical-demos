...
| Queue: `LifeUnderwritingCases` | Canonical demo case, idempotency, state transitions, outputs, ordered audit events, and retry ownership. | New Orchestrator queue in the solution deployment beneath approved parent `JD_Demos/demos`; not provisioned. | Least-privilege robot access, masked identifiers, configured retention, immutable evidence/decision history, and version-conflict handling. |
...
| Solution boundary and delivery contract | One `life-insurance-underwriting-evidence-exception` solution, one `.uipx`, nested Flow layout, resource refresh, immutable version, changed-solution CI, and approved deployment parent `JD_Demos/demos`. | Fully specified; non-Azure resources require provisioning. |
...
10. Refresh solution resources, restore, dry-run pack, and register immutable deployment configuration under approved parent `JD_Demos/demos` after human-owned policies are approved.
