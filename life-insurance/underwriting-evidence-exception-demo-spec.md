...
| Decide and review | **3. Recommend and underwrite** | Deterministic rules send only clean, authorized, complete offer cases to the synthetic straight-through route. Other material cases set the record to `awaiting_primary_review`, and the senior underwriter decides in the `Underwriting Evidence Console`. The Flow waits at the record-updated event node and resumes from the persisted decision. Postpone or decline then creates the `Underwriting Adverse Control Review` Action Center task for a distinct supervisor. | `straightThroughEligible === true && evidenceComplete === true && materialInconsistency === false && extractionConfidence >= 0.90 && recommendation.route === "OFFER" && recommendation.confidence >= 0.90` permits the synthetic straight-through offer route. The wait node filters on the literal submitted status, and the next decision compares the event record ID with the instance record ID; a mismatch loops back to the wait node on a labelled edge. Human routes then use the persisted `humanDecision.outcome`; `POSTPONE` and `DECLINE` require `secondReview.result === "approved"`. Thresholds are demo configuration, not carrier policy claims. |
...
2. The filter is one literal value: `caseStatus` equals the `primary_decision_submitted` value of `LiUnderwritingStatus`. Filters accept literals only, so the node cannot filter on this instance's record ID. The literal form for a choice field, integer `numberId` or value name, is not verified; confirm it when configuring the node. If a choice field cannot be filtered reliably, add a dedicated `decisionSubmitted` BOOL marker field and filter on that instead.
3. The next decision compares the event payload record ID with the instance record ID. A mismatch takes a labelled `Other case` edge back to the same wait node. A match reads the full record with `get-entity-record-by-id`, because `MULTILINE_MAX` fields return only a size marker on list and query reads.
...
7. Bind the record-created trigger and the record-updated wait node with `--detail.objectName LifeUnderwritingCase` and the `demos` Data Fabric connection. Confirm the literal filter form for the choice field, then wire the status filter or the marker-field fallback, the record-ID comparison, the labelled loop-back edge, and the `error` handle. Record the resolved event mode.
