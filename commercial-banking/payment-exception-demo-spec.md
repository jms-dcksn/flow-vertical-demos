...
Every Flow write uses `uipath.connector.uipath-uipath-dataservice.update-entity-record`, and the console writes through the SDK single-record update. Both are single-record writes, so both fire record-updated events. The wait node's literal status filter is what keeps an intake or assessment write from resuming a case.
...
1. The wait node filter is the literal `Status` value `review_submitted`. Event filters accept literals only, so the node cannot filter on this instance's record ID. The literal form for a choice field, integer `numberId` or value name, is not verified; confirm it when configuring the node. If a choice field cannot be filtered reliably, add a dedicated `DecisionSubmitted` BOOL marker field and filter on that instead.
2. When the event fires, the Flow compares the payload record ID with its own record ID.
3. A mismatch takes a labelled `Other case` edge straight back into the same wait node. A match reads the record with `get-entity-record-by-id` and routes from `ReviewOutcome`.
...
| Wait-node correlation isolation | A reviewer decision on one case cannot resume another case. | Update a second record to `review_submitted` while a case waits: the waiting instance stays waiting, takes the `Other case` edge, and resumes only on its own record ID. |
